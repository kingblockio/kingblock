...
func (ctx CoreContext) Ensure(name string) (CoreContext, error) {
	...
	logger.Info("==============Ensure", "xxxx", 1)

	err := EnsureAccountExists(ctx, name)
	if err != nil {
	logger.Info("==============Ensure", "xxxx", 2)
		return ctx, err
	}

	ctx, err = EnsureAccountNumber(ctx)
	if err != nil {
	logger.Info("==============Ensure", "xxxx", 3)
		return ctx, err
	}
	// default to next sequence number if none provided
	ctx, err = EnsureSequence(ctx)
	if err != nil {
	logger.Info("==============Ensure", "xxxx", 4)
		return ctx, err
	}


	logger.Info("==============Ensure", "xxxx", 5)
	logger.Info("==============Ensure", "AccountNumber", ctx.AccountNumber)
	logger.Info("==============Ensure", "Sequence", ctx.Sequence)
	return ctx, nil
}
...
func (ctx CoreContext) SignAndBuild(name, passphrase string, msgs []sdk.Msg, cdc *wire.Codec) ([]byte, error) {
        ...
        logger := log.NewTMLogger(log.NewSyncWriter(os.Stdout))
        logger.Info("==============SignAndBuild", "xxxx", 1)
	...
	memo := ctx.Memo
        logger.Info("==============AccountNumber", "number", accnum)
