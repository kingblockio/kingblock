...
[website](https://kingblockio.network/docs/getting-started/full-node.html#run-a-full-node).

The rest of this folder is moved to the [testnets
repo](https://github.com/kingblockio/testnets).
...
possible. In the meantime, join the [Validator Chat](https://riot.im/app/#/room/#kingblockio_validators:matrix.org)
for technical support, and [open issues](https://github.com/kingblockio/kingblock) if you run into any! Thanks very much for your patience and support. :)
...
```bash
mkdir -p $GOPATH/src/github.com/kingblockio
cd $GOPATH/src/github.com/kingblockio
git clone https://github.com/kingblockio/kingblock
cd kingblock && git checkout v0.23.1
...
View the status of the network with the [Cosmos Explorer](https://explorekingblockio.network). Once your full node syncs up to the current block height, you should see it appear on the [list of full nodes](https://explorekingblockio.network/validators). If it doesn't show up, that's ok--the Explorer does not connect to every node.
...
- `kingblockioaccaddr`
  ...
  * e.g. `kingblockioaccaddr15h6vd5f0wqps26zjlwrc6chah08ryu4hzzdwhc`

- `kingblockioaccpub`
  * Derived from account keys generated by `gaiacli keys add`
  * e.g. `kingblockioaccpub1zcjduc3q7fu03jnlu2xpl75s2nkt7krm6grh4cc5aqth73v0zwmea25wj2hsqhlqzm`

- `kingblockiovalpub`
  * Generated when the node is created with `gaiad init`.
  * Get this value with `gaiad tendermint show_validator`
  * e.g. `kingblockiovalpub1zcjduc3qcyj09qc03elte23zwshdx92jm6ce88fgc90rtqhjx8v0608qh5ssp0w94c`
...
The best way to get tokens is from the [Cosmos Testnet Faucet](https://faucetkingblockio.network). If the faucet is not working for you, try asking [#kingblockio-validators](https://riot.im/app/#/room/#kingblockio-validators:matrix.org). The faucet needs the `kingblockioaccaddr` from the account you wish to use for staking.

After receiving tokens to your address, you can view your account's balance by typing:

```bash
gaiacli account <account_kingblockioaccaddr>
```

> _*Note:*_ When you query an account balance with zero tokens, you will get this error: `No account with address <account_kingblockioaccaddr> was found in the state.` This can also happen if you fund the account before your node has fully synced with the chain. These are both normal. Also, we're working on improving our error messages!

## Run a Validator Node

[Validators](https://kingblockio.network/validators) are responsible for committing new blocks to the blockchain through voting. A validator's stake is slashed if they become unavailable, double sign a transaction, or don't cast their votes. If you only want to run a full node, a VM in the cloud is fine. However, if you are want to become a validator for the Hub's `mainnet`, you should research hardened setups. Please read [Sentry Node Architecture](https://forum.kingblockio.network/t/sentry-node-architecture-overview/454) to protect your node from DDOS and ensure high-availability. Also see the [technical requirements](https://github.com/kingblockio/kingblockio/blob/master/VALIDATORS_FAQ.md#technical-requirements)). There's also more info on our [website](https://kingblockio.network/validators).

### Create Your Validator

Your `kingblockiovalpub` can be used to create a new validator by staking tokens. You can find your validator pubkey by running:
...
> _*NOTE:*_  Don't use more `steak` thank you have! You can always get more by using the [Faucet](https://faucetkingblockio.network/)!

```bash
gaiacli stake create-validator \
  --amount=5steak \
  --pubkey=$(gaiad tendermint show_validator) \
  --address-validator=<account_kingblockioaccaddr>
...
gaiacli stake edit-validator
  --address-validator=<account_kingblockioaccaddr>
  --moniker="choose a moniker" \
  --website="https://kingblockio.network" \
...
You should also be able to see your validator on the [Explorer](https://explorekingblockio.network/validators). You are looking for the `bech32` encoded `address` in the `~/.gaiad/config/priv_validator.json` file.
...
Wait for your full node to catch up to the latest block. Next, run the following command. Note that `<kingblockioaccaddr>` is the address of your validator account, and `<name>` is the name of the validator account. You can find this info by running `gaiacli keys list`.

```bash
gaiacli stake unrevoke <kingblockioaccaddr> --chain-id=gaia-7001 --from=<name>
```
...
On the upcoming mainnet, you can delegate `Atom` to a validator. These [delegators](https://kingblockio.network/resources/delegators) can receive part of the validator's fee revenue. Read more about the [Cosmos Token Model](https://github.com/kingblockio/kingblockio/raw/master/Cosmos_Token_Model.pdf).
...
gaiacli stake delegate \
  --amount=10steak \
  --address-delegator=<account_kingblockioaccaddr> \
  --address-validator=<validator_kingblockioaccaddr> \
  --from=<key_name> \
  --chain-id=gaia-7001
```
...
For more information about the governance process and how it works, please check out the Governance module [specification](https://github.com/kingblockio/kingblock/tree/develop/docs/spec/governance).
...
gaiacli gov submit-proposal \
  ...
  --proposer=<account_kingblockioaccaddr> \
  --deposit=<40steak> \
  --from=<name> \
  --chain-id=gaia-7001
```
...
gaiacli gov deposit \
  ...
  --depositer=<account_kingblockioaccaddr> \
  --deposit=<200steak> \
  --from=<name> \
  --chain-id=gaia-7001
```
...
gaiacli gov vote \
  --proposalID=<proposal_id> \
  --voter=<account_kingblockioaccaddr> \
...
gaiacli send \
  ...
  --chain-id=gaia-7001 \
  --from=<key_name> \
  --to=<destination_kingblockioaccaddr>
```
...
```bash
gaiacli account <account_kingblockioaccaddr>
gaiacli account <destination_kingblockioaccaddr>
```

You can also check your balance at a given block by using the `--block` flag:

```bash
gaiacli account <account_kingblockioaccaddr> --block=<block_height>
```
